...
浏览器每次拿到返回的请求结果时，会根据服务器响应的缓存策略（如 `Cache-Control`、`Expires`）决定是否将结果和缓存标识存入浏览器缓存中。后续发起同样请求时会尝试命中缓存读取；未命中缓存时，浏览器会直接请求服务器并依据响应头决定后续缓存行为。
...
**强缓存**：浏览器每次发起请求，都会先在浏览器缓存中查找该请求的结果以及缓存标识，尝试直接复用，不真实发起此次请求到服务端；

**协商缓存**：强制缓存失效后，浏览器携带协商缓存标识向服务器发起请求，由服务器根据缓存标识决定是否使用缓存的过程（协商）；
...
- `max-age=`xxx (xxx is numeric)：缓存内容将在xxx秒后失效，尝试协商缓存

### 协商缓存

**缓存头header字段**：`Last-Modified/If-Modified-Since` 和 `ETag/If-None-Match`
